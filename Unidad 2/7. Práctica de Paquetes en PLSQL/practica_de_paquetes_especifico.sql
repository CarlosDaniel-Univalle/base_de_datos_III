----En esta practica he decido hacer utilizado datos con codigos previamente desarrollado
...
CREATE OR REPLACE PACKAGE REGIONES AS
  ...
  FUNCTION EXISTE_REGION(codigo IN NUMBER) RETURN BOOLEAN;
END REGIONES;
/


--Cuerpo del paquete "REGIONES" e implementa los procedimientos y funciones:
...
CREATE OR REPLACE PACKAGE BODY REGIONES AS
  ...
  FUNCTION CON_REGION(codigo IN NUMBER) RETURN VARCHAR2 IS
    nombre_region VARCHAR2(100);
  BEGIN
    SELECT nombre INTO nombre_region FROM tabla_de_regiones WHERE codigo = codigo;
...
--Cuerpo del paquete "NOMINA" e implementa las funciones sobrecargadas:
